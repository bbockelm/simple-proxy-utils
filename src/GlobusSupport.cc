...
static void globus_print(globus_result_t result, char **err_msg) {
  if (!err_msg) return;

  globus_object_t *error_obj = globus_error_get(result);
  if (error_obj == GLOBUS_ERROR_NO_INFO) {
    *err_msg = strdup("Globus error occurred (no further information available)");
    return;
  }
  char *error_full = globus_error_print_chain(error_obj);
  if (!error_full) {
    *err_msg = strdup("Globus error occurred (error occurred when generating error string)");
    return;
  }
  std::string msg_full = "Globus error: ";
  msg_full += error_full;
  *err_msg = strdup(msg_full.c_str());
  free(error_full);
}
...
bool globus_activate(char **err_msg) {
  ...
  if (GLOBUS_SUCCESS != globus_thread_set_model("pthread")) {
    if (err_msg) *err_msg = strdup("Failed to enable Globus thread model.");
    return false;
  }
  if (GLOBUS_SUCCESS != globus_module_activate(GLOBUS_GSI_CERT_UTILS_MODULE)) {
    if (err_msg) *err_msg = strdup("Failed to activate Globus GSI cert utils module.");
    return false;
  }
  if (GLOBUS_SUCCESS != globus_module_activate(GLOBUS_GSI_CREDENTIAL_MODULE)) {
    if (err_msg) *err_msg = strdup("Failed to activate Globus GSI credential module.");
    globus_module_deactivate(GLOBUS_GSI_CERT_UTILS_MODULE);
    return false;
  }
  if (GLOBUS_SUCCESS != globus_module_activate(GLOBUS_GSI_CALLBACK_MODULE)) {
    if (err_msg) *err_msg = strdup("Failed to activate Globus GSI callback module.");
    globus_module_deactivate(GLOBUS_GSI_CERT_UTILS_MODULE);
  ...
  if (GLOBUS_SUCCESS != globus_module_activate(GLOBUS_GSI_SYSCONFIG_MODULE)) {
    if (err_msg) *err_msg = strdup("Failed to activate Globus GSI sysconfig module.");
    globus_module_deactivate(GLOBUS_GSI_CERT_UTILS_MODULE);
    globus_module_deactivate(GLOBUS_GSI_CREDENTIAL_MODULE);
  ...
  if (GLOBUS_SUCCESS != result) {
    if (err_msg) {
      std::string err_full = "Failed to determine trusted certificates directory: ";
      char *err_sub_msg = NULL;
      globus_print(result, &err_sub_msg);
      err_full += err_sub_msg;
      free(err_sub_msg);
      *err_msg = strdup(err_full.c_str());
    }
...
class VerifyCtx {
  ...
  globus_result_t operator() (globus_gsi_cred_handle_t cred_handle, char **err_msg)
  {
...
globus_result_t
Verify::operator() (globus_gsi_cred_handle_t cred_handle, char** err_msg)
{
  return m_ctx(cred_handle, err_msg);
}
...
bool globus_verify(X509* cert, STACK_OF(X509)* chain, char** dn, char** err_msg)
{
  ...
  try {
    ...
    result = verify(state.m_cred, err_msg);
  } catch (globus_result_t result) {
    if (err_msg) {
      std::string err_full = "Failed to create verification context: ";
      char *err_sub_msg = NULL;
      globus_print(result, &err_sub_msg);
      err_full += err_sub_msg;
      free(err_sub_msg);
  ...
  if (GLOBUS_SUCCESS != result) {
    if (err_msg) {
      std::string err_full = "Failed to validate credentials: ";
      char *err_sub_msg = NULL;
      globus_print(result, &err_sub_msg);
      err_full += err_sub_msg;
      free(err_sub_msg);
  ...
  if (!dn) {
    if (err_msg) {
      std::string err_full = "Unable to determine certificate DN: ";
      char openssl_err_str[120];
      int err_code = ERR_get_error();
      char *openssl_err_ptr = ERR_error_string(err_code, openssl_err_str);
      err_full += openssl_err_ptr ? openssl_err_ptr : "(unable to determine OpenSSL error)";
      *err_msg = strdup(err_full.c_str());
    }
    return false;
  }
...
bool globus_get_cert_and_chain(const char * creds, size_t credslen, X509 **cert, STACK_OF(X509) **chain, char **err_msg)
{
  ...
  if (!state.m_bio) {
    if (err_msg) {
      std::string err_full = "Unable to allocate new BIO object: ";
      char openssl_err_str[120];
      int err_code = ERR_get_error();
      char *openssl_err_ptr = ERR_error_string(err_code, openssl_err_str);
      err_full += openssl_err_ptr ? openssl_err_ptr : "(unable to determine OpenSSL error)";
